...
""" DEM and GSD"""
# Inputs filenames
zDEM = r'DEMs\filled_lc3_dem.txt'  # ASCII raster DEM containing the bed surface elevation
rainfall_time_series = 'climate/test_storm.txt'
gsd = pd.read_excel('GSDs/LC3_large_grain_size_dist.xlsx', sheet_name='GSD', skiprows=0).values
bed_surf__gsd_loc_node = pd.read_excel('GSDs/lc3_gsd_locations.xlsx')
...
t = 0
max_dt = 0.1  # Use seconds
sim_max_t = 50 + max_dt
save_data_time_interval = 20
dt_precision = 3
...
node_list = np.array([33999 ,6301])               # Location of the nodes where information for postprocess will be extracted 
...
of = OverlandFlow(grid, mannings_n=n, rainfall_intensity=0.0, alpha=0.25, steep_slopes=True, theta=1.0)
...
of.overland_flow(dt=0.1)  # Use a small dt just to initiate the calculation
print(f"Initial of.dt: {of.dt}")  # Check what dt is set to

if 'surface_water__depth' not in grid.at_node:
    grid.add_zeros('surface_water__depth', at='node')

# Add a thin layer of water (e.g., 1 mm)
grid.at_node['surface_water__depth'] += 0.01  # water depth in meters
"""
# Check and adjust dt
...
# Visualization of Sediment Transport Overlaid on DEM
dem_data = grid.at_node['topographic__elevation'].reshape(grid.shape)
largest_sediment_data = grid.at_node['sediment__size_class_largest'].reshape(grid.shape)

fig, ax = plt.subplots(figsize=(10, 7))
dem_plot = ax.imshow(dem_data, cmap='gray', origin='lower')
fig.colorbar(dem_plot, ax=ax, fraction=0.03, pad=0.04, label='Elevation (m)')
sediment_plot = ax.imshow(largest_sediment_data, cmap='viridis', origin='lower', alpha=0.7)
fig.colorbar(sediment_plot, ax=ax, fraction=0.03, pad=0.04, label='Sediment Concentration')
ax.set_title('DEM with Overlay of Largest Sediment Class Moved')
